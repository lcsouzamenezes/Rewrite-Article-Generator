...
//explode function with multiple options in $delimiter = array();
...
//Find synonyms by word on thesaurus.com
private function findSynonymOnWeb($word){
	...
	//QUERY FOR THE OLD VERSION (2018) (Keep it just in case)
...
private function getSynonymByWord($words,$sentences,$level){
	...
	for($a=0; $a < count($sentences); $a++){
		for($b=0; $b < count($words[$a]); $b++){
			//Don't need to find sysnonyms for the words with less than 4 chars (by default) can be changed by the user in adv customization
...
//Will update this function in the future to get better result (like try to analyze the context etc..) 
